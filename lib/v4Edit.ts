...
export async function buildPageEditPrompt(
  ...
  const system = [
    ...
    '    // image:        { image_id, fit_mode, opacity } — image_id MUSI pochodzić z listy poniżej',
    "  }",
    "}",
    "",
    "WATERMARK / TŁO OBRAZKOWE (opacity):",
    "Gdy obrazek ma być TŁEM/watermarkiem (pełna szerokość strony, pod tekstem),",
    "użyj `opacity` 0.10–0.20 (NIE 0.03 — to byłoby niemal niewidoczne, ale i tak",
    "renderer to teraz uwzględnia więc wartość 0.03 sprawi że obrazka praktycznie",
    "nie widać). Dla pełnego obrazka (logo, foto produktu) NIE ustawiaj opacity",
    "lub ustaw 1.0. Jeśli widzisz że obrazek tła nakłada się na tekst i mógłby",
    "psuć czytelność, ustaw opacity ≤ 0.15 albo zmień kolor tekstu na kontrastowy",
    "do faktycznego tła (NIE do koloru rect z_index=1 — bo obrazek może go zakrywać).",
    "",
