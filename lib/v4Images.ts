...
export function renderImagesGalleryForPrompt(images: Array<ProjectImage & { anthropic_file_id: string }>): string {
  ...
  const lines: string[] = [
    ...
    "konkretne ID ponizej.",
    "",
    "Properties dla image:",
    "  - image_id: ID z listy ponizej (string, wymagany)",
    "  - fit_mode: 'contain' (mieszczone, default) lub 'cover' (wypelnia, przycina)",
    "  - opacity: 0.0-1.0 (default 1.0). Dla WATERMARKA uzyj 0.10-0.20.",
    "  - grayscale: true/false (default false). Gdy user prosi o 'czarno-bialy',",
    "    'bez kolorow', 'grayscale' — ustaw `grayscale: true`. Renderer wymusi",
    "    desaturacje (CSS filter w editorze, konwersja bytes w PDF).",
    "",
