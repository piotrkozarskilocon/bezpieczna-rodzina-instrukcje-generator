  ...
  return (
    <div className="mx-auto max-w-6xl px-6 py-12">
            ...
            <li key={p.id}>
              ...
              >
                ...
                <p className="mt-1 text-xs text-slate-500">
                  {p.pages_count > 0 ? `${p.pages_count} stron` : "Pusty projekt"}
                  {" · "}
                  <span>{p.default_lang.toUpperCase()}</span>
                  {p.reference_pdf_path && <span> · PDF ref ✓</span>}
                </p>
      ...
      <p className="mt-10 text-xs uppercase tracking-[0.16em] text-slate-400">
        Edytor strukturalny — buduj instrukcję od szablonów.
      </p>
    </div>
  );
}
...
function UploadForm({ onUploaded }: { onUploaded: () => void }): React.ReactElement {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      // Step 1 — register project (with optional fileSize → signed upload URL).
      setStage("Tworzenie projektu...");
      const body = file
        ? { name: name.trim(), fileSize: file.size }
        : { name: name.trim() };
      ...
      const result = (await initRes.json()) as {
        id: string;
        uploadUrl?: string | null;
      };

      // default_lang stays "pl" — output language is chosen only at export
      // time when the user converts the manual into another locale.

      if (file && result.uploadUrl) {
        // Step 2 — direct PUT to Supabase Storage with progress.
        setStage("Wgrywanie PDF...");
        await uploadWithProgress(result.uploadUrl, file, (pct) => setProgress(pct));

        // Step 3 — finalize (verify upload + commit reference_pdf_path).
        setStage("Finalizacja...");
        const finRes = await fetch(`${API_BASE}/projects/${result.id}/finalize`, {
          method: "POST",
        });
        if (!finRes.ok) {
    ...
    <form onSubmit={submit} className="mb-6 rounded-xl border border-slate-200 bg-white p-6">
      ...
      >
        ...
        <p className="text-sm font-medium text-slate-700">
          {file ? file.name : "PDF referencyjny — opcjonalnie (przeciągnij lub kliknij)"}
        </p>
        {file && (
          <p className="mt-1 text-xs text-slate-500">
        ...
        {!file && (
          <p className="mt-1 text-xs text-slate-400">
            Możesz utworzyć pusty projekt bez PDF — referencja służy tylko do podglądu.
          </p>
        )}
      </label>
      ...
      <div className="mt-4">
        ...
        <input
          ...
          placeholder="np. GJD.15 — QSG+KG"
          className="mt-1 block w-full rounded-md border border-slate-300 px-3 py-2 text-sm shadow-sm focus:border-slate-500 focus:outline-none focus:ring-1 focus:ring-slate-500"
          required
        />
        <p className="mt-1 text-[11px] text-slate-400">
          Język bazowy projektu zawsze PL — wybór języka wyjściowego nastąpi przy eksporcie.
        </p>
      </div>
