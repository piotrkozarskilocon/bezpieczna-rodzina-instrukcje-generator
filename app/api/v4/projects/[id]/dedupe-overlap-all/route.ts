...
export async function POST(request: NextRequest, ctx: RouteContext) {
  ...
  const { data: pages } = await sb
    .from("gen4_pages")
    .select("id, page_number, height_mm")
  ...
  const perPage: Array<{ page_number: number; groups_before: number; patches: number; groups_after: number }> = [];
  ...
  for (const page of pages) {
    ...
    if (groupsBefore.length === 0) {
      perPage.push({ page_number: page.page_number, groups_before: 0, patches: 0, groups_after: 0 });
      continue;
    }
