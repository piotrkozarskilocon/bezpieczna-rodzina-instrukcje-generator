...
/**
 * Structured editor (v3) — build pages from templates and elements.
 *
 * Layout (CSS grid):
 *   ┌────────┬─────────────────────────┬──────────────┐
...
const API = "/generator-instrukcji/api/v3";
          ...
          <ul className="space-y-1">
            ...
            {pages.map((p) => (
              <li key={p.id}>
                ...
                >
                  <span>
                    <span className="font-mono font-semibold">#{p.page_number}</span>{" "}
                    <span className="text-slate-500">{p.template ?? "blank"}</span>
                  </span>
                  <button
                    type="button"
